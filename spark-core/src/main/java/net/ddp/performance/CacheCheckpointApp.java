...
public class CacheCheckpointApp {
    ...
    private void start() {
        ...
        this.spark = SparkSession.builder().appName("Example of cache and checkpoint").master("local[*]")
    ...
    private long processDataframe(int recordCount, Mode mode) {
        ...
        log.info("Five-star publications per year");
        for (Row r : yearDf) {
            log.info("{} -- {}", r.getInt(0), r.getLong(1));
        }
