...
public class FlattenJsonApp {
    ...
    private void start() {
        ...
        SparkSession spark = SparkSession.builder().appName("Automatic flattening of a JSON document").master("local").getOrCreate();

        // Reads a JSON, stores it in a dataframe
        Dataset<Row> invoicesDf = spark.read().format("json").option("multiline", true).load("data/json/nested_array.json");
    ...
    public static Dataset<Row> flattenNestedStructure(SparkSession spark, Dataset<Row> df) {
        ...
        for (StructField field : fields) {
            ...
            case ARRAY_TYPE:
                ...
                processedDf = processedDf.withColumn(field.name(), explode(processedDf.col(field.name() + "_tmp")));
            ...
            case STRUCT_TYPE:
                ...
                for (StructField structField : structFields) {
                    processedDf = processedDf.withColumn(field.name() + "_" + structField.name(), processedDf.col(field.name() + "." + structField.name()));
                }
