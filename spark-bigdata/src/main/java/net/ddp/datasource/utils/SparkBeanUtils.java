package net.ddp.datasource.utils;
...
import net.ddp.datasource.annotations.SparkColumn;
...
public class SparkBeanUtils {
    ...
    /**
     * Builds a schema from the bean. The resulting schema is not directly usable by
     * Spark as it is a super set of what is needed.
    ...
    public static Schema getSchemaFromBean(Class<?> beanClass) {
        ...
        for (int i = 0; i < methods.length; i++) {
            ...
            if (sparkColumn == null) {
                ...
                log.debug("Annotation for method {}, column name is {}", methodName, columnName);
                ...
                case "null":
                    ...
                    break;
                default:
                    log.debug("Will infer data type from return type for column {}", columnName);
                    dataType = getDataTypeFromReturnType(method);
                }
    ...
    public static Row getRowFromBean(Schema schema, Object bean) {
        ...
        for (int i = 0; i < fieldName.length; i++) {
            ...
            try {
                ...
                log.error("The method {} does not exists: {}.", methodName, e.getMessage());
                return null;
            } catch (SecurityException e) {
                log.error("The method {} raised a security concern: {}.", methodName, e.getMessage());
                return null;
            }
            try {
                cells.add(method.invoke(bean));
            } catch (IllegalAccessException e) {
                log.error("The method {} is not accessible: {}.", methodName, e.getMessage());
                return null;
            } catch (IllegalArgumentException e) {
                // this should never happen as we have check that the getter has no
                // argument, but we never know, right?
                log.error("The method {} is expecting arguments: {}.", methodName, e.getMessage());
                return null;
            } catch (InvocationTargetException e) {
                log.error("The method {} raised an invocation target issue: {}.", methodName, e.getMessage());
                return null;
            }
        }
